<template>
	<div class="container1">
		<div>
			<DeleteMyInfoModal :deleteMyInfoModal="deleteMyInfoModal" @modalClose="toggleDeleteMyInfoModal" :userId="userId" />
		</div>
		<div>
			<form method="POST" autocomplete="off" @submit.prevent="onSubmit" class="myInfo-form">
				<!-- 타이틀 -->
				...
				<div class="info-input-container">
					...
					<div class="input-area">
						<input class="input-text readonly-input" type="text" id="userId" readonly :value="userId" />
				...
				<div class="info-input-container">
					...
					<div class="input-area">
						<input class="input-text" type="text" name="nickname" id="nickname" placeholder="한글, 영문 대소문자, 숫자만 사용 가능" :value="nickname" />
						<button class="input-in-btn" id="nickname-check" @click="nicknameCheck">중복확인</button>
				...
				<div class="info-input-container">
					...
					<div class="input-area">
						<input class="input-text" type="text" name="name" id="name" placeholder="이름 입력" :value="name" />
					</div>
					<div class="error-msg-area">
          ...
          <div class="error-msg-area">
            <p style="display: none" id="Code-msg" class="msg"></p>
				...
				<div class="myinfo-btn-area">
					<input type="button" id="backbtn" @click="backbtn" value="뒤로가기" class="big-ctlbtn cancle-btn" />
					<input type="button" id="update-my-info" @click="submit" value="정보수정" class="big-ctlbtn select-btn" />

					<input type="button" id="delete-my-info" @click="toggleDeleteMyInfoModal" value="탈퇴하기" class="big-ctlbtn delete-btn" />
				</div>
			</form>
		</div>
	</div>
...
export default {
	...
	setup() {
		...
		const getUserData = () => {
			...
			axios
				.post(`http://${process.env.VUE_APP_BACK_END_URL}/member/userData`, userIdData, {
					headers: {
	...
	methods: {
		...
		nicknameCheck() {
			...
			if (nickname === "") {
				document.getElementById("nicknameCheck-msg").innerHTML = "닉네임은 필수 입력 사항입니다.";
			...
			if (nickname >= 20) {
				document.getElementById("nicknameCheck-msg").innerHTML = "닉네임은 20자리 이하로 입력해주세요.";
				document.getElementById("nicknameCheck-msg").style.display = "block";
				return false;
			} else {
				document.getElementById("nicknameCheck-msg").style.display = "none";
			...
			// 닉네임 중복확인 API 호출
			axios
				.get(`http://${process.env.VUE_APP_BACK_END_URL}/api/auth/${nickname}/nicknameCheck`)
				.then((res) => {
					if (res.data == 0) {
						document.getElementById("nicknameCheck-msg").innerHTML = "사용 가능한 닉네임입니다.";
						document.getElementById("nicknameCheck-msg").style.display = "block";
						document.getElementById("nicknameCheck-msg").style.color = "green";
						this.nicknameCheckBoolean = true;
					} else {
						document.getElementById("nicknameCheck-msg").innerHTML = "이미 사용중인 닉네임입니다.";
						document.getElementById("nicknameCheck-msg").style.display = "block";
					}
				})
				.catch((error) => {
		...
		submit() {
			...
			if (name === "") {
				document.getElementById("nameCheck-msg").innerHTML = "이름은 필수 입력 사항입니다.";
			...
			if (nicknameRegex.test(nickname) === false) {
				...
				document.getElementById("idCheck-msg").innerText = "아이디 중복확인을 해주세요.";
			...
			//   });
			axios
				.post(`http://${process.env.VUE_APP_BACK_END_URL}/api/auth/updateMyInfo`, this.formData, {
					headers: {
						"Content-Type": "application/json",
					},
				})
				.then((res) => {
					alert("내 정보가 수정되었습니다. 다시 로그인 해주세요.");
					if (res.data == 1) {
            axios.get(`http://${process.env.VUE_APP_BACK_END_URL}/api/auth/logout`).then((res) => {
              if (res.data == "Logout") {
                localStorage.removeItem("jwtToken");
