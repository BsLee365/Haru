...
public interface RecommendPlaceRepository extends JpaRepository<Place, Long> {
    ...
    @Query(nativeQuery = true, value = "select a.place_num as placeNum , a.place_address as placeAddress, a.place_img as placeImg, a.place_link as placeLink," +
            ...
            " minus" +
            " select p.place_num as placeNum, p.place_address as placeAddress, p.place_img as placeImg, p.place_link as placeLink," +
            " p.place_name as placeName, m.user_id as userId, p.place_score as placeScore" +
            " from place p join (select m.user_id, w.place_num" +
            " from wishlist w join member m" +
            " on w.user_id = m.user_id) m" +
            " on p.place_num = m.place_num")
